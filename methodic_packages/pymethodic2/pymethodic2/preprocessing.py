...
def clean_data(thisdata):
    ...
    '''
    utils.print_helper("Cleaning data", level=1)
    thisdata = thisdata.copy().dropna(subset=[columns.raw_record_type, columns.raw_date_logged])
    thisdata = thisdata.copy().drop_duplicates(ignore_index=True)
    if len(thisdata) == 0:
        return (thisdata)
    thisdata = thisdata.copy().loc[thisdata['interaction_type'] != 'Usage Stat']
    if not columns.timezone in thisdata.keys() or any(thisdata[columns.timezone] == None):
        utils.print_helper("WARNING: Record has no timezone information.  Registering reported time as UTC.")
        thisdata[columns.timezone] = "UTC"
    if not 'application_label' in thisdata.columns:
        thisdata['application_label'] = ""
    thisdata['application_label'] = thisdata['application_label'].fillna("")
    thisdata = thisdata[
        ['study_id', 'participant_id', 'application_label', columns.full_name, columns.prep_record_type,
         columns.raw_date_logged, columns.timezone]].copy()
    # fill timezone by preceding timezone and then backwards
    thisdata = thisdata.sort_values(by=[columns.raw_date_logged]). \
        reset_index(drop=True). \
        fillna(method="ffill").fillna(method="bfill").copy()
    thisdata['date_tzaware'] = thisdata.copy().apply(utils.get_dt,
                                              axis=1)  # transforms string to a timestamp, returns local time
    thisdata['action'] = thisdata.copy().apply(utils.get_action, axis=1)
    # time.sleep(5)
    thisdata = thisdata.copy().sort_values(by=[columns.raw_date_logged, 'action']).reset_index(drop=True)
...
def extract_usage(dataframe, precision=3600):
    ...
    cols = ['participant_id',
            ...
            'interaction_type']  # was 'app_data_type'
    other_interactions = {
        interactions.screen_non_interactive1: "Screen Non-interactive",  # if you left it running
        interactions.screen_non_interactive2: "Screen Non-interactive",
        interactions.screen_interactive1: "Screen Interactive",
        interactions.screen_interactive2: "Screen Interactive",
        interactions.notification_seen: "Notification Seen",
        interactions.notification_interruption: "Notification Interruption",
        interactions.power_off: "Power Off",
        interactions.start_up: "Device Start up",
        interactions.user_interaction: "User Interaction"
    }
    ...
    for idx, row in rawdata.iterrows():
        ...
        curtime = row.event_timestamp  # date_tzaware

        # make dict entry of every app that was moved to foreground. Time is local.
        if interaction in interactions.foreground:
        ...
        if interaction in interactions.background:
            ...
            if latest_unbackgrounded and app == latest_unbackgrounded['unbgd_app']:
                ...
                if timediff < timedelta(seconds=1):
                    timepoints = get_timestamps.run(curtime, latest_unbackgrounded['unbgd_time'],
                                                    precision=precision, row=row, localtz=row.timezone)

                    timepoints[columns.prep_record_type] = 'App Usage'
                    timepoints['timezone'] = row.timezone
                    timepoints['study_id'] = row.study_id
                    timepoints['application_label'] = row.application_label
            ...
            if app in openapps.keys() and openapps[app]['open'] == True:
                ...
                timepoints = get_timestamps.run(curtime, prevtime, precision=precision, row=row, localtz=row.timezone)
        ...
        if interaction in other_interactions.keys():
            timepoints = get_timestamps.run(curtime, precision=precision, row=row, localtz=row.timezone)
    ...
    if len(alldata) > 0:
        ...
        cols_to_select.extend(['timezone', 'application_label', columns.flags])
...
def preprocess_dataframe(dataframe, precision=3600, sessioninterval=[5 * 60]):
    ...
    utils.print_helper("LOG: Extracting usage...", level=1)
    ...
    if not isinstance(tmp, pd.DataFrame) or np.sum(tmp[columns.prep_duration_seconds]) == 0:
        utils.print_helper(
            f"WARNING: Person {dataframe['participant_id'].iloc[0]} does contain time usage for the interactions studied.  Skipping...")
        return None
    utils.print_helper("LOG: checking overlap session...", level=1)
    ...
    data_fordownload = data[['study_id', 'participant_id', 'interaction_type', 'application_label', 'app_package_name',
                             'app_datetime_start', 'app_datetime_end', 'timezone', 'app_duration_seconds',
                             'day', 'weekdayMF', 'weekdayMTh', 'weekdaySTh',
                             'app_engage_30s', 'app_switched_app', 'app_usage_flags']]
    # RENAME to old column names for consistency for clients
    data_fordownload = data_fordownload.rename(columns={'interaction_type': 'app_record_type',
                                     'application_label': 'app_title',
                                     'app_full_name': 'app_package_name',
                                     'timezone': 'app_timezone'})
    # print(data_fordownload.columns)
