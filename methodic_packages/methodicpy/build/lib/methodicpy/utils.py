...
def query_usage_table(start, end, filters, engine, counting=False, users=[], studies=[]):
    ...
    if counting:
        ...
        output = pd.read_sql_query(data_query, engine, index_col=None)

    # print(f"Output of query is of type {type(output)}")
    return output


@task(name='query_export_table2')
...
@task(name='fill_appcat_quarterly2')
...
@task(name='add_session_durations2')
...
@task(name='backwards_compatibility2')
...
@task(name='round_down_to_quarter2')
