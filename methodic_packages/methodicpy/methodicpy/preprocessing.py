...
def clean_data(thisdata):
    ...
    thisdata = thisdata.dropna(subset=['interaction_type', 'event_timestamp']).copy()
    thisdata = thisdata.drop_duplicates(ignore_index=True).copy()
    if len(thisdata) == 0:
        return (thisdata)
    thisdata = thisdata.loc[thisdata['interaction_type'] != 'Usage Stat']
    if not 'timezone' in thisdata.keys() or any(thisdata['timezone'] == None):
        utils.print_helper.run("WARNING: Record has no timezone information.  Registering reported time as UTC.")
        thisdata['timezone'] = "UTC"
    ...
    thisdata = thisdata[
        ['study_id', 'participant_id', 'application_label', 'app_package_name', 'interaction_type', 'event_timestamp',
         'timezone']].copy()
    # fill timezone by preceding timezone and then backwards
    thisdata = thisdata.sort_values(by=['event_timestamp']). \
...
def preprocess_dataframe(dataframe, precision=3600, sessioninterval=[5 * 60]):
    ...
    data_fordownload.rename(columns={'interaction_type': 'app_record_type',
                                     'application_label': 'app_title',
                                     'app_full_name': 'app_package_name',
                                     'timezone': 'app_timezone'}, inplace=True)

    data_fordownload.sort_values(by=['app_datetime_start'], axis=1, inplace=True)
