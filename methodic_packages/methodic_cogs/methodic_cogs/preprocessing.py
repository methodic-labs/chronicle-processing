...
def extract_usage(dataframe,precision=3600):
    ...
    for idx, row in rawdata.iterrows():
        ...
        if interaction == 'Move to Background':
            ...
            if latest_unbackgrounded and app == latest_unbackgrounded['unbgd_app']:
                ...
                if timediff < timedelta(seconds=1):
                    timepoints = get_timestamps(curtime, latest_unbackgrounded['unbgd_time'],
            ...
            if app in openapps.keys() and openapps[app]['open'] == True:
                ...
                timepoints = get_timestamps(curtime, prevtime, precision=precision, row=row, localtz=row.app_timezone)
        ...
        if interaction in other_interactions.keys():
            timepoints = get_timestamps(curtime, precision=precision, row=row, localtz=row.app_timezone)
...
@task(name='check_overlap_add_sessions2')
...
@task(name='log_exceed_durations_minutes2')
...
@flow(name='preprocess_dataframe2')
def preprocess_dataframe(dataframe, precision=3600,sessioninterval = [5*60]):
    ...
    tmp = extract_usage(dataframe,precision=precision)   #HERE get app durations
    if not isinstance(tmp,pd.DataFrame) or np.sum(tmp[columns.prep_duration_seconds]) == 0:
        utils.print_helper(f"WARNING: Person {dataframe['participant_id'].iloc[0]} does not seem to contain relevant data.  Skipping...")
        return None
    utils.print_helper("LOG: checking overlap session...", level=1)
    data = check_overlap_add_sessions(tmp,session_def=sessioninterval)
    data = utils.add_warnings(data) #Add flags for long usage or usage gaps
...
@flow(name='get_person_preprocessed_data2')
def get_person_preprocessed_data(person, datatable) -> pd.DataFrame:
    '''
    ...
    df_prep = preprocess_dataframe(df, sessioninterval = [30])
